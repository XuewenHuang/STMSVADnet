...
class train_datas(Dataset):
    def __init__(self, train_config):
        ...
        self.short_len = train_config.short_len
        ...
        for folder in sorted(glob.glob(f'{train_config.train_dataset_path}/*')):
            ...
            for i in range(0, len(one_folder_frame_imgs) - 1 - self.clip_length - self.short_len):
                one_clip = []
                for index in range(i, i + self.clip_length+1):
                    # print(one_folder_frame_imgs[i])
                    one_clip.append(one_folder_frame_imgs[index])
                one_clip.append(one_folder_frame_imgs[index+self.short_len-1])
...
class Label_loader:
    ...
    def load_shanghaitech(self):
        np_list = glob.glob(f'/home/hxw/Python/TFUnet/datasets/shanghaitech/testing/test_frame_mask/*.npy')
        np_list.sort()
